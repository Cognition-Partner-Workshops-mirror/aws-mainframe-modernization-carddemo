"""
PySpark script to parse the COBOL fixed-width card data file
(``app/data/ASCII/carddata.txt``) using the copybook-derived schema
from ``CVACT02Y.cpy``.

Record layout: CARD-RECORD  (150 bytes per record)
Key type-mapping decisions:
  - PIC X(16)  → StringType   (card number kept as string for leading zeros)
  - PIC 9(11)  → LongType     (unsigned numeric display, 11-digit account ID)
  - PIC 9(03)  → IntegerType  (unsigned numeric display, 3-digit CVV)
  - PIC X(n)   → StringType   (alphanumeric, trailing spaces trimmed)

No signed-decimal (PIC S9…V99) fields exist in CVACT02Y.
...
SCHEMA_FILE = os.path.join(
    PROJECT_ROOT, "pyspark_copybook_parser", "schemas", "CVACT02Y_schema.json"
)
VALIDATION_OUTPUT = os.path.join(
    PROJECT_ROOT, "pyspark_copybook_parser", "validation", "carddata_validation.txt"
)
...
# No signed-decimal columns in CVACT02Y — all fields are PIC X or PIC 9
...
validation_lines.append("=" * 80)
validation_lines.append(f"Copybook:           CVACT02Y.cpy")
validation_lines.append(f"Record layout:      CARD-RECORD (150 bytes)")
validation_lines.append(f"Data file:          app/data/ASCII/carddata.txt")
